...
#1번
    def search_train(self):
        ...
        if search_train_menu == "1":
            ...
            a, b = divmod(tmp_mytime, 100) #divmod : 몫a, 나머지b
            mytime = a * 60 + b #하면 time_list가 나옴
    ...
    def my_reserved_train(self):
        ...
        if my_reserved_train_menu == "1":
            ...
            while i < len(reservation_list):
                ...
                if ind is not None or reservation_list is not 0: #예매내역이 있으면
                    cancel = input("\n☆ 예매를 취소하시겠습니까? [Y/N]"
                                   "\n☆ 뒤로가기"
                                   "\n입력 : ")
...
def close_time(mytime, time_list):  # 근접한 값 찾는 함수
    ...
    abs_list = []
    for i in range(len(b)): #네 타임밖에 없음
        abs_list.append(abs(a - b[i]))
        #한 번씩 빼보고
    ind = abs_list.index(min(abs_list))
    #가장 차이가 적은것을 골라서
    return real_time_list[ind] #실제 기차시간표로 반환
